...
def cmd_monitor(command, command_dir='.',
    ...
    callbacks : functions
        iterable of functions with signature
        def callback(command, stdout):
            return bool
        where command is the same as this method's command argument,
        stdout is the standard output of the command,
        and the return is whether or not to prevent a notification from
        happening.
    success_sound : str
        sound to play when the command is successful (set to '' for no sound)
    fail_sound : str
        sound to play when the command is unsuccessful (set to '' for no sound)
    notify_time_secs : float
        how many seconds to keep the desktop notification up
